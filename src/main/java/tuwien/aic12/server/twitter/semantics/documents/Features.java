...
public class Features {
    ...
    /**
     * sets a given map that associates a feature's id to the frequency of the
     * feature
     *
     * @param f2freq a map that associates a feature's id to the frequency of
     * the feature
     */
     ...
     *
     * @return a map that associates a feature's id to the frequency of the
     * feature
     */
    ...
    /**
     * creates a map that associates a feature's id to the frequency of the
     * feature
     */
        ...
        for (Integer doc : ds.getD2f_train().keySet()) {
            for (Integer feat : ds.getD2f_train().get(doc)) {
                if (!f2freqMap.containsKey(feat)) {
                    f2freqMap.put(feat, 1);
                } else {
                    app = f2freqMap.get(feat);
                    app++;
                    f2freqMap.put(feat, app);
                }
            }
        }
        this.f2freq = f2freqMap;
    }

    /**
     * creates new maps that contains only features that appear a given number
     * of times
     *
     * @param number the number of times that a feature must appear in the
     * dataset
     */
